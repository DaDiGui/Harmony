...
public class Equipment
{
    ...
    public (bool isUnlocked, EntitlementSetEnum set) HasCosmeticUnlocked(ItemClass itemClass)
    {
        ...
        if (itemClass.SDCSData != null)
        {
            ...
            if (entitlementSetEnum != 0 && EntitlementManager.Instance.HasEntitlement(entitlementSetEnum))
            {
    ...
    public bool HasAnyItems(int ignoreIndex = -1)
    {
        for (int i = 0; i < m_slots.Length; i++)
        {
            if (i != ignoreIndex && m_slots[i] != null)
            {
                return true;
            }
        }
    ...
    public static Equipment Read(BinaryReader reader)
    {
        ...
        if (num >= 2)
        {
            for (int j = 0; j < equipment.m_cosmeticSlots.Length; j++)
            {
                ...
                {
                    equipment.m_cosmeticSlots[j] = ItemClass.GetItemClass(CosmeticMappingIDString[num2]);
                }
            }
