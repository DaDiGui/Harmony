...
public class SdtdConsole : SingletonMonoBehaviour<SdtdConsole>
{
    ...
    public void Update()
    {
        ...
        lock (m_commandsToExecuteAsync)
        {
            try
            {
                CommandSenderInfo commandSenderInfo = default(CommandSenderInfo);
                commandSenderInfo.IsLocalGame = false;
                commandSenderInfo.RemoteClientInfo = null;
                commandSenderInfo.NetworkConnection = m_commandsToExecuteAsync[0].sender;
                CommandSenderInfo senderInfo = commandSenderInfo;
    ...
    public void RegisterCommands()
    {
        ...
        try
        {
            foreach (IConsoleCommand value2 in commandsList.Values)
            {
                ...
                for (int i = 0; i < value2.GetCommands().Length; i++)
                {
                    string text = value2.GetCommands()[i];
    ...
    {
        CommandSenderInfo commandSenderInfo = default(CommandSenderInfo);
        commandSenderInfo.IsLocalGame = _cInfo == null;
        commandSenderInfo.RemoteClientInfo = _cInfo;
        commandSenderInfo.NetworkConnection = null;
        CommandSenderInfo senderInfo = commandSenderInfo;
