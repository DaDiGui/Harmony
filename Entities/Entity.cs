...
public class Entity : MonoBehaviour
{
    ...
    public static float[] waterLevelDirOffsets = new float[6]
    {
        Mathf.Cos(0f),
        Mathf.Sin(0f),
        Mathf.Cos((float)Math.PI * 2f / 3f),
        Mathf.Sin((float)Math.PI * 2f / 3f),
    ...
    public virtual string EntityName => Localization.Get(EntityClass.list[entityClass].entityClassName);
    ...
    public void SetupBounds()
    {
        ...
        {
            nativeCollider = (Collider)(object)component;
    ...
    public virtual void updateTransform()
    {
        ...
        {
            float y;
            if ((bool)(UnityEngine.Object)(object)physicsRB)
            {
    ...
    public void FixedUpdate()
    {
        ApplyFixedUpdate();
        wasFixedUpdate = true;
        if (!(UnityEngine.Object)(object)physicsRB)
        {
            return;
        }

        Rigidbody obj = physicsRB;
        obj.velocity *= 0.9f;
        Rigidbody obj2 = physicsRB;
        obj2.angularVelocity *= 0.9f;
        ...
        if ((bool)(UnityEngine.Object)(object)physicsCapsuleCollider)
        {
    ...
    public virtual void AddCharacterController()
    {
        ...
        if (flag2)
        {
            Collider obj = nativeCollider;
            BoxCollider val = (BoxCollider)(object)((obj is BoxCollider) ? obj : null);
            if ((bool)(UnityEngine.Object)(object)val && linkCapsuleSizeToBoundingBox)
            {
                num2 = Utils.FastMax(val.size.y - num, stepHeight);
                center = val.center;
                center.y = num2 * 0.5f;
                if (val.size.x > val.size.y)
                {
                    center.y += (val.size.x - val.size.y) * 0.5f;
                }

                num3 = val.size.x * 0.5f - num;
                flag = true;
            }
        }
        ...
        boundingBox = BoundsUtils.BoundsForMinMax(-scaledExtent, scaledExtent);
        if ((bool)(UnityEngine.Object)(object)nativeCollider)
        {
    ...
    public void PhysicsInit()
    {
        ...
        if (parent != transform2.parent)
        {
            ...
            for (int num = componentsInChildren.Length - 1; num >= 0; num--)
            {
                ...
                CapsuleCollider val2;
                BoxCollider val3;
                SphereCollider val4;
                if ((bool)(UnityEngine.Object)(object)(val2 = (CapsuleCollider)(object)((val is CapsuleCollider) ? val : null)))
                {
                    val2.center = (val2.center + localPosition) * x;
                    val2.height *= x;
                    val2.radius *= x;
                }
                else if ((bool)(UnityEngine.Object)(object)(val3 = (BoxCollider)(object)((val is BoxCollider) ? val : null)))
                {
                    val3.center = (val3.center + localPosition) * x;
                    val3.size *= x;
                }
                else if ((bool)(UnityEngine.Object)(object)(val4 = (SphereCollider)(object)((val is SphereCollider) ? val : null)))
                {
                    val4.center = (val4.center + localPosition) * x;
                    val4.radius *= x;
                }
            }
        ...
        physicsRB.collisionDetectionMode = (CollisionDetectionMode)2;
        physicsTargetPos = physicsPos;
        CapsuleCollider component = physicsRBT.GetComponent<CapsuleCollider>();
        if ((bool)(UnityEngine.Object)(object)component && component.direction == 1)
        {
    ...
    public virtual void PhysicsPush(Vector3 forceVec, Vector3 forceWorldPos, bool affectLocalPlayerController = false)
    {
        ...
        if ((bool)(UnityEngine.Object)(object)val)
        {
            ...
            {
                val.AddForceAtPosition(forceVec, forceWorldPos - Origin.position, (ForceMode)1);
            }
            else
            {
                val.AddForce(forceVec, (ForceMode)1);
            }
        }
    }
    ...
    {
        if ((position - physicsMasterSendPos).sqrMagnitude < 0.00250000018f && Quaternion.Angle(qrotation, physicsMasterSendRot) < 1f)
        {
            return null;
        }
    ...
    public void PhysicsMasterSendToServer(Transform t)
    {
        if (clientEntityId == 0)
        {
            ...
            if (GetVelocityPerSecond().sqrMagnitude < 0.160000011f)
            {
    ...
    public void SetScale(float scale)
    {
        ...
        {
            if (((Joint)val).autoConfigureConnectedAnchor)
            {
                ((Joint)val).autoConfigureConnectedAnchor = false;
                ((Joint)val).autoConfigureConnectedAnchor = true;
            }
        }
        ...
        if ((bool)physicsRBT)
        {
            ...
            {
                Collider val2 = componentsInChildren2[num];
                CapsuleCollider val3 = (CapsuleCollider)(object)((val2 is CapsuleCollider) ? val2 : null);
                if (val3 != null)
                {
                    val3.center *= scale;
                    val3.height *= scale;
    ...
    public void ReplicateSpeeds()
    {
        ...
        if (num * num + num2 * num2 >= 4.00000044E-06f)
        {
            speedSentTicks = 3;
    ...
    public void ccEntityCollisionResults()
    {
        ...
        motion.x = vector3.x;
        motion.z = vector3.z;
        isCollidedHorizontally = (collisionFlags & 1) > 0;
        isCollidedVertically = (collisionFlags & 6) > 0;
    ...
    public virtual void entityCollision(Vector3 _motion)
    {
        ...
        {
            if (!(UnityEngine.Object)(object)emodel.pelvisRB)
            {
                return;
            }
    ...
    public virtual void SetVelocityPerSecond(Vector3 vel, Vector3 angularVel)
    {
        ...
        if (isPhysicsMaster && (bool)(UnityEngine.Object)(object)physicsRB)
        {
    ...
    public virtual AttachedToEntitySlotExit FindValidExitPosition(List<AttachedToEntitySlotExit> candidatePositions)
    {
        ...
        for (int i = 0; i < candidatePositions.Count; i++)
        {
            for (float num3 = 0f; num3 < 0.75f; num3 += 0.24f)
            {
                ...
                {
                    vector4.y = vector2.y + num5;
                    flag = Physics.Raycast(vector4, normalized, num4, 1084817408);
                    if (flag)
                    {
                        break;
                    }
                ...
                Vector3 vector5 = vector2 - normalized * 0.1f;
                Vector3 vector6 = vector5;
                vector6.y += num2;
                if (!flag && !Physics.CapsuleCast(vector5, vector6, num, normalized, num4, 1084817408))
                {
    ...
    public void SetPhysicsCollidersLayer(int layer)
    {
        ...
        {
            ((Component)(object)componentsInChildren[i]).gameObject.layer = layer;
        }
    }
