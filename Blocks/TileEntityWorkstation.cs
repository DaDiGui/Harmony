...
public class TileEntityWorkstation : TileEntity
{
    ...
    public override void OnSetLocalChunkPosition()
    {
        ...
        if (block.Properties.Values.ContainsKey("Workstation.InputMaterials"))
        {
            ...
            if (text.Contains(","))
            {
                materialNames = text.Replace(" ", "").Split(',', StringSplitOptions.None);
            }
            else
            {
        ...
        if (block.Properties.Values.ContainsKey("Workstation.Modules"))
        {
            ...
            string[] array2 = ((!text2.Contains(",")) ? new string[1] { text2 } : text2.Replace(" ", "").Split(',', StringSplitOptions.None));
    ...
    public override void write(PooledBinaryWriter _bw, StreamModeWrite _eStreamMode)
    {
        ...
        ((BinaryWriter)_bw).Write((byte)49);
        switch (_eStreamMode)
        {
            case StreamModeWrite.Persistency:
                {
                    ((BinaryWriter)_bw).Write(lastTickTime);
                    ...
                    ((BinaryWriter)_bw).Write(isBurning);
                    ((BinaryWriter)_bw).Write(currentBurnTimeLeft);
                    int num3 = currentMeltTimesLeft.Length;
                    ((BinaryWriter)_bw).Write((byte)num3);
                    for (int k = 0; k < num3; k++)
                    {
                        ((BinaryWriter)_bw).Write(currentMeltTimesLeft[k]);
                    }

                    ((BinaryWriter)_bw).Write(isPlayerPlaced);
            ...
            case StreamModeWrite.ToServer:
                {
                    ...
                    ((BinaryWriter)_bw).Write((byte)num2);
                    for (int j = 0; j < num2; j++)
                    {
                        ((BinaryWriter)_bw).Write(currentMeltTimesLeft[j]);
                    }

                    ((BinaryWriter)_bw).Write(isPlayerPlaced);
                    ((BinaryWriter)_bw).Write(GameTimer.Instance.ticks - lastTickTime);
                    writeItemStackArray(_bw, lastInput);
                    break;
                }
            case StreamModeWrite.ToClient:
                {
                    ...
                    ((BinaryWriter)_bw).Write((byte)num);
                    for (int i = 0; i < num; i++)
                    {
                        ((BinaryWriter)_bw).Write(currentMeltTimesLeft[i]);
                    }
