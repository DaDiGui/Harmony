...
public class ItemClass : ItemData
{
    ...
    public virtual void Init()
    {
        ...
        {
            string[] array = Properties.Values[PropGroupName].Split(',', StringSplitOptions.None);
            if (array.Length != 0)
            {
        ...
        if (SoundTick != null)
        {
            string[] array2 = SoundTick.Split(',', StringSplitOptions.None);
            SoundTick = array2[0];
            if (array2.Length >= 2)
            {
    ...
    public void LateInit()
    {
        ...
        RecipeUnlockData[] array = (IsBlock() ? GetBlock().UnlockedBy : UnlockedBy);
        for (int i = 0; i < array.Length; i++)
        {
            if (array[i].UnlockType == RecipeUnlockData.UnlockTypes.None)
            {
                Log.Error("Error loading item class '" + base.Name + "'. Property UnlockedBy refers to '" + Properties.Values[Block.PropUnlockedBy] + "' which is neither a perk nor item.");
            }
        }
    }
    ...
    public virtual Transform CloneModel(World _world, ItemValue _itemValue, Vector3 _position, Transform _parent, BlockShape.MeshPurpose _purpose = BlockShape.MeshPurpose.World, TextureFullArray _textureFullArray = default(TextureFullArray))
    {
        ...
        if (CanHold())
        {
            ...
            if (gameObject == null)
            {
                gameObject = LoadManager.LoadAsset<GameObject>("@:Other/Items/Crafting/leather.fbx", null, null, _deferLoading: false, _loadSync: true).Asset;
            }
        }
