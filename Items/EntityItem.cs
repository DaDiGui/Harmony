...
public class EntityItem : Entity
{
    ...
    public override void OnUpdateEntity()
    {
        ...
        if (isPhysicsMaster && !SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && ((uint)ticksExisted & (true ? 1u : 0u)) != 0)
        {
    ...
    public virtual void createMesh()
    {
        ...
        {
            Collider val = componentsInChildren[num2];
            Rigidbody component = ((Component)(object)val).gameObject.GetComponent<Rigidbody>();
            if (((bool)(UnityEngine.Object)(object)component && component.isKinematic) || (val is MeshCollider && !((MeshCollider)val).convex))
            {
                val.enabled = false;
            }
            else
            {
                ((Component)(object)val).gameObject.layer = 13;
                val.enabled = true;
                flag = false;
                NGUITools.AddMissingComponent<RootTransformRefEntity>(((Component)(object)val).gameObject);
            }
    ...
    public override void AddVelocity(Vector3 _vel)
    {
        ...
        if (isPhysicsMaster)
        {
            ...
            itemRB.AddForce(_vel * 6f, (ForceMode)1);
        }
    }
    ...
    public override void VisiblityCheck(float _distanceSqr, bool _masterIsZooming)
    {
        if ((bool)itemTransform && meshRenderers != null)
        {
            ...
            if (ticksExisted < 3 && _distanceSqr < 0.640000045f)
            {
                flag = false;
            }
    ...
    public void OnCollisionEnter(Collision collision)
    {
        ...
        {
            ContactPoint val = contactPoints[num];
            if (Utils.FastAbs(Vector3.Dot(collision.relativeVelocity, ((ContactPoint)(ref val)).normal)) < 1f)
            {
                continue;
            }

            Entity hitRootEntity = GameUtils.GetHitRootEntity(((Component)(object)((ContactPoint)(ref val)).otherCollider).transform.tag, ((Component)(object)((ContactPoint)(ref val)).otherCollider).transform);
            ...
            Vector3i pos = World.worldToBlockPos(((ContactPoint)(ref val)).point - 0.25f * ((ContactPoint)(ref val)).normal + Origin.position);
            BlockValue block = world.GetBlock(pos);
            if (block.isair)
            {
                WorldRayHitInfo worldRayHitInfo = new WorldRayHitInfo();
                GameUtils.FindMasterBlockForEntityModelBlock(world, -((ContactPoint)(ref val)).normal, ((Component)(object)((ContactPoint)(ref val)).otherCollider).transform.tag, ((ContactPoint)(ref val)).point + Origin.position, ((Component)(object)((ContactPoint)(ref val)).otherCollider).transform, worldRayHitInfo);
            ...
            float num2 = Utils.FastAbs(((ContactPoint)(ref val)).normal.x);
            float num3 = Utils.FastAbs(((ContactPoint)(ref val)).normal.y);
            float num4 = Utils.FastAbs(((ContactPoint)(ref val)).normal.z);
            BlockFace side = BlockFace.Top;
            if (num2 >= num3 && num2 >= num4)
            {
                if (((ContactPoint)(ref val)).normal.x < 0f)
                {
                    side = BlockFace.East;
                }
                else if (((ContactPoint)(ref val)).normal.x > 0f)
                {
                    side = BlockFace.West;
                }
            }
            ...
            {
                if (((ContactPoint)(ref val)).normal.z < 0f)
                {
                    side = BlockFace.North;
                }
                else if (((ContactPoint)(ref val)).normal.z > 0f)
                {
                    side = BlockFace.South;
                }
            }
            else if (((ContactPoint)(ref val)).normal.y < 0f)
            {
    ...
    public void CheckStick(Collision collision)
    {
        ...
        Rigidbody obj = itemRB;
        obj.velocity *= num;
        Rigidbody obj2 = itemRB;
        obj2.angularVelocity *= num;
        if (stickPercent >= 1f && !stickT)
        {
            ...
            Collider[] componentsInChildren = ((Component)(object)itemRB).GetComponentsInChildren<Collider>();
            for (int num2 = componentsInChildren.Length - 1; num2 >= 0; num2--)
            {
                ((Component)(object)componentsInChildren[num2]).gameObject.layer = 0;
            }
