...
public static class WorldStaticData
{
    ...
    public static IEnumerator Init(bool _bForce, bool _bDediServer, ProgressDelegate _progressDelegate)
    {
        ...
        {
            for (int k = 0; k < meshDescCol.meshes.Length; k++)
            {
                meshDescCol.meshes[k].TexDiffuse = null;
                meshDescCol.meshes[k].TexNormal = null;
                meshDescCol.meshes[k].TexSpecular = null;
                meshDescCol.meshes[k].TexEmission = null;
                meshDescCol.meshes[k].TexHeight = null;
                meshDescCol.meshes[k].TexOcclusion = null;
                meshDescCol.meshes[k].TexMask = null;
                meshDescCol.meshes[k].TexMaskNormal = null;
            }
        }
    ...
    public static void Reset(string _xmlNameContaining)
    {
        ...
        DeflateOutputStream zipStream = new DeflateOutputStream((Stream)memoryStream, 3);
        XmlLoadInfo[] array = xmlsToLoad;
        foreach (XmlLoadInfo xmlLoadInfo in array)
        {
            if (!string.IsNullOrEmpty(_xmlNameContaining) && !xmlLoadInfo.XmlName.ContainsCaseInsensitive(_xmlNameContaining))
    ...
    public static IEnumerator cacheSingleXml(XmlLoadInfo _loadInfo, XmlFile _origXml, MemoryStream _memStream, DeflateOutputStream _zipStream)
    {
        ...
        _origXml.SerializeToStream((Stream)(object)_zipStream, _minified: true);
        _zipStream.Restart();
    ...
    public static IEnumerator LoadAllXmlsCo(bool _isStartup, ProgressDelegate _progressDelegate)
    {
        ...
        DeflateOutputStream zipStream = new DeflateOutputStream((Stream)memStream, 3);
        XmlLoadInfo[] array = xmlsToLoad;
        foreach (XmlLoadInfo xmlLoadInfo in array)
        {
            if (!xmlLoadInfo.XmlFileExists())
    ...
    public static void SaveXmlsToFolder(string _exportPath)
    {
        ...
        foreach (XmlLoadInfo xmlLoadInfo in array)
        {
            ...
            {
                DeflateInputStream val = new DeflateInputStream((Stream)memoryStream);
                try
                {
                    using MemoryStream memoryStream2 = new MemoryStream();
                    StreamUtils.StreamCopy((Stream)(object)val, memoryStream2);
    ...
    public static IEnumerator handleReceivedConfigs()
    {
        ...
        while (waitingFor < xmlsToLoad.Length)
        {
            ...
            XmlFile xmlFile;
            using (MemoryStream memoryStream = new MemoryStream(loadInfo.CompressedXmlData))
            {
                DeflateInputStream val = new DeflateInputStream((Stream)memoryStream);
                try
                {
                    xmlFile = new XmlFile((Stream)(object)val);
                }
                finally
                {
                    ((IDisposable)val)?.Dispose();
