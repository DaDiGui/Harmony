...
public class NetPackageWeather : NetPackage
{
    ...
    public override void write(PooledBinaryWriter _bw)
    {
        ...
        {
            WeatherPackage weatherPackage = weatherPackages[i];
            ((BinaryWriter)_bw).Write(weatherPackage.biomeId);
            ((BinaryWriter)_bw).Write(weatherPackage.groupIndex);
            ((BinaryWriter)_bw).Write(weatherPackage.remainingSeconds);
            for (int j = 0; j < weatherPackage.param.Length; j++)
            {
                ((BinaryWriter)_bw).Write(weatherPackage.param[j]);
            }
        }
    }
