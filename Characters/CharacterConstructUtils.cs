...
public static class CharacterConstructUtils
{
    ...
    public static void Stitch(GameObject sourceObj, GameObject parentObj, SDCSUtils.TransformCatalog boneCatalog, Material eyeMat = null)
    {
    ...
    public static void setupEquipment(GameObject _rig, SDCSUtils.TransformCatalog _boneCatalog, List<SDCSUtils.SlotData> slotData, bool _ignoreDlcEntitlements)
    {
        ...
        foreach (SDCSUtils.SlotData slotDatum in slotData)
        {
            ...
            if ((bool)transform2)
            {
                _ = slotDatum.CullDistance;
                Stitch(transform2.gameObject, _rig, _boneCatalog);
            }
        }
    ...
    public static void setupHairObjects(GameObject _rig, SDCSUtils.TransformCatalog _boneCatalog, string hairName, string mustacheName, string chopsName, string beardName)
    {
        ...
        {
            string text = baseHairColorLoc + "/" + archetype.HairColor + ".asset";
            ScriptableObject scriptableObject = DataLoader.LoadAsset<ScriptableObject>(text);
            if (scriptableObject == null)
            {
                Log.Warning("SDCSUtils::" + text + " not found for hair color " + archetype.HairColor + "!");
            }
            else
            {
