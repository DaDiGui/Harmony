...
public class Inventory
{
    ...
    public IEnumerator delayedShowHideHeldItem(bool hideFirst, float waitTime)
    {
        ...
        ItemValue currentItemValue = holdingItemItemValue;
        yield return new WaitForSeconds(Mathf.Max(waitTime, 0.3f));
        if (currentItemValue == holdingItemItemValue && previousHeldItemValue != holdingItemItemValue && !holdingItem.Equals(bareHandItem))
        {
    ...
    public int GetBestQuickSwapSlot()
    {
        ...
        if (slots[quickSwapSlotIdx].itemValue.Equals(quickSwapItemValue))
        {
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            if (slots[i].itemValue.Equals(quickSwapItemValue))
            {
                return i;
            }
        }
