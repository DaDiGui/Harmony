...
public class NetPackagePlayerInventory : NetPackage
{
    ...
    {
        base.write(_writer);
        ((BinaryWriter)_writer).Write(toolbelt != null);
        if (toolbelt != null)
        {
            GameUtils.WriteItemStack(_writer, toolbelt);
        }

        ((BinaryWriter)_writer).Write(bag != null);
        if (bag != null)
        {
            GameUtils.WriteItemStack(_writer, bag);
        }

        ((BinaryWriter)_writer).Write(equipment != null);
        if (equipment != null)
        {
            ...
            {
                ((BinaryWriter)_writer).Write(cosmeticIDs[i]);
            }

            List<int> unlockedCosmetics = equipment.m_unlockedCosmetics;
            ((BinaryWriter)_writer).Write(unlockedCosmetics.Count);
            for (int j = 0; j < unlockedCosmetics.Count; j++)
            {
                ((BinaryWriter)_writer).Write(unlockedCosmetics[j]);
            }
        }

        ((BinaryWriter)_writer).Write(dragAndDropItem != null);
