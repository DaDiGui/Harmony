...
public class XUiC_WorkstationOutputWindow : XUiController
{
    ...
    {
        base.Update(_dt);
        if (!base.xui.playerUI.windowManager.IsInputActive() && (((OneAxisInputControl)base.xui.playerUI.playerInput.GUIActions.LeftStick).WasPressed || ((OneAxisInputControl)base.xui.playerUI.playerInput.PermanentActions.Reload).WasPressed))
        {
            controls.MoveAll();
        }
    }
