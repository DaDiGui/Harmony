...
public class XUiC_DataManagementBar : XUiController
{
    ...
    public struct BarRegion : IEquatable<BarRegion>
    {
    ...
    public void Refresh()
    {
        ...
        if (displayMode != 0 && displayMode == DisplayMode.Preview)
        {
            RefreshPreviewMode();
        }
        else
    ...
    public void RefreshSelectionMode()
    {
        ...
        [PublicizedFrom(EAccessModifier.Private)]
        void GetPixelValues(BarRegion byteRegion, int maxPosition, out int position, out int width)
        {
            position = Mathf.Min(Mathf.FloorToInt(bytesToPixels * (float)byteRegion.Start), maxPosition - 8);
        ...
        void UpdateRegion(BarRegion byteRegion, XUiV_Sprite fillSprite, XUiV_Sprite outlineSprite, SelectionDepth depth)
        {
            ...
            {
                GetPixelValues(hoveredByteRegion, maxPosition2, out var position2, out var width2);
                bar_hovered_outline.IsVisible = true;
                bar_hovered_outline.Position = new Vector2i(position2, 0);
                bar_hovered_outline.Size = new Vector2i(width2, fullHeight);
            }

            if (byteRegion.Size > 0)
            {
                GetPixelValues(byteRegion, maxPosition2, out var position3, out var width3);
                fillSprite.IsVisible = true;
                fillSprite.Position = new Vector2i(position3, 0);
                fillSprite.Size = new Vector2i(width3, fullHeight);
                ...
                {
                    GetPixelValues(archivePreviewByteRegion, maxPosition2, out var position4, out var width4);
                    bar_pending.IsVisible = true;
                    bar_pending.Position = new Vector2i(position4, 0);
                    bar_pending.Size = new Vector2i(width4, fullHeight);
                }

                maxPosition2 = position3 + width3;
            }
        }
    }
