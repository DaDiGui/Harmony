...
public class XUiC_ToolTip : XUiController
{
    ...
    public string ToolTip
    {
        ...
        set
        {
            ...
            if (label != null && (Object)(object)label.Label != null)
            {
                label.Label.overflowMethod = (Overflow)2;
                label.Text = tooltip;
    ...
    public override void Update(float _dt)
    {
        ...
        if (tooltip != "")
        {
            ...
            border.Size = new Vector2i(((UIWidget)label.Label).width + 18, ((UIWidget)label.Label).height + 12);
            background.Size = new Vector2i(border.Size.x - 6, border.Size.y - 6);
            Vector2i xUiScreenSize = base.xui.GetXUiScreenSize();
            if (((UIWidget)label.Label).width > xUiScreenSize.x / 4)
            {
                label.Label.overflowMethod = (Overflow)3;
                ((UIWidget)label.Label).width = xUiScreenSize.x / 4 - 10;
            }
            else
            {
