...
public class XUiC_Chat : XUiController
{
    ...
    public void registerRegularChatMessageHandlers()
    {
        ...
        static string GetTargetDisplayNameRegular(EChatType _chatType, string _targetId)
        {
            ...
            if (!int.TryParse(_targetId, out var result2))
            {
                throw new ArgumentException("Could not parse chat entity id '" + _targetId + "'");
            }

            if (!GameManager.Instance.World.Players.dict.TryGetValue(result2, out var value))
            {
                return Localization.Get("xuiChatTarget" + _chatType.ToStringCached());
            }
        ...
        static bool IsValidTargetRegular(EChatType _chatType, string _targetId)
        {
            ...
            if (!int.TryParse(_targetId, out var result3))
            {
                throw new ArgumentException("Could not parse chat entity id '" + _targetId + "' for target validation");
            }

            if (!GameManager.Instance.World.Players.dict.TryGetValue(result3, out var _))
            {
                return false;
            }

            return result3 != GameManager.Instance.World.GetPrimaryPlayer().entityId;
        }
    ...
    public override void UpdateInput()
    {
        ...
        if (PlatformManager.NativePlatform.Input.CurrentInputStyle == PlayerInputManager.InputStyle.Keyboard)
        {
            ...
            if (((OneAxisInputControl)gUIActions.Up).WasPressed)
            {
                cbxTarget.ChangeIndex(-1);
            }

            if (((OneAxisInputControl)gUIActions.Down).WasPressed)
            {
                cbxTarget.ChangeIndex(1);
            }
        }

        if (((OneAxisInputControl)base.xui.playerUI.playerInput.PermanentActions.Cancel).WasPressed)
        {
