...
public class XUiM_PlayerInventory : XUiModel, IInventory
{
    ...
    public bool AddItemsUsingPreferenceTracker(XUiC_ItemStackGrid _srcGrid, PreferenceTracker preferences)
    {
        ...
        for (int l = 0; l < itemStackControllers.Length; l++)
        {
            (bool anyMoved, bool allMoved) tuple = TryStackItem(0, itemStackControllers[l].ItemStack);
            ...
            if (item)
            {
                if (item2)
                {
                    itemStackControllers[l].ItemStack = ItemStack.Empty;
                }
                else
                {
                    hashSet.Add(itemStackControllers[l].ItemStack.itemValue.type);
                }

                _srcGrid.HandleSlotChangedEvent(l, itemStackControllers[l].ItemStack);
            }
        }
        ...
        if (preferences.toolbelt != null)
        {
            ...
            int k;
            for (k = 0; k < preferences.toolbelt.Length && k < slots.Length; k++)
            {
                if (slots[k].IsEmpty())
                {
                    XUiC_ItemStack xUiC_ItemStack = null;
                    int type2 = preferences.toolbelt[k].itemValue.type;
                    xUiC_ItemStack = ((!hashSet.Contains(type2)) ? itemStackControllers.FirstOrDefault([PublicizedFrom(EAccessModifier.Internal)] (XUiC_ItemStack stack) => stack.ItemStack.Equals(preferences.toolbelt[k])) : itemStackControllers.FirstOrDefault([PublicizedFrom(EAccessModifier.Internal)] (XUiC_ItemStack stack) => stack.ItemStack.itemValue.type == type2));
                    if (xUiC_ItemStack?.ItemStack != null && !xUiC_ItemStack.ItemStack.IsEmpty())
                    {
                        toolbelt.SetItem(k, xUiC_ItemStack.ItemStack);
        ...
        if (preferences.equipment != null)
        {
            ...
            for (j = 0; j < num; j++)
            {
                if (localPlayer.equipment.GetSlotItem(j) != null)
                {
                    continue;
                }

                XUiC_ItemStack xUiC_ItemStack2 = itemStackControllers.FirstOrDefault([PublicizedFrom(EAccessModifier.Internal)] (XUiC_ItemStack stack) => stack.ItemStack.itemValue.Equals(preferences.equipment[j]));
                if (xUiC_ItemStack2?.ItemStack != null && !xUiC_ItemStack2.ItemStack.IsEmpty())
                {
                    localPlayer.equipment.SetSlotItem(j, xUiC_ItemStack2.ItemStack.itemValue);
        ...
        if (preferences.bag != null)
        {
            ...
            int i;
            for (i = 0; i < preferences.bag.Length && i < slots2.Length; i++)
            {
                if (slots2[i].IsEmpty())
                {
                    XUiC_ItemStack xUiC_ItemStack3 = null;
                    int type = preferences.bag[i].itemValue.type;
                    xUiC_ItemStack3 = ((!hashSet.Contains(type)) ? itemStackControllers.FirstOrDefault([PublicizedFrom(EAccessModifier.Internal)] (XUiC_ItemStack stack) => stack.ItemStack.Equals(preferences.bag[i])) : itemStackControllers.FirstOrDefault([PublicizedFrom(EAccessModifier.Internal)] (XUiC_ItemStack stack) => stack.ItemStack.itemValue.type == type));
                    if (xUiC_ItemStack3?.ItemStack != null && !xUiC_ItemStack3.ItemStack.IsEmpty())
                    {
                        backpack.SetSlot(i, xUiC_ItemStack3.ItemStack);
    ...
    {
        return AddItem(_itemStack, playCollectSound: true);
    }
