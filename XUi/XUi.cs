...
public class XUi : MonoBehaviour
{
    ...
    public float GetPixelRatioFactor()
    {
        if (pixelRatioFactor == 0f || Screen.height != lastScreenHeight)
        {
            ...
            pixelRatioFactor = UIRoot.pixelSizeAdjustment / xuiGlobalScaling / @float / activeUiScale;
    ...
    public void SetScale(float scale = -1f)
    {
        ...
        float @float = GamePrefs.GetFloat(EnumGamePrefs.OptionsScreenBoundsValue);
        float activeUiScale = GameOptionsManager.GetActiveUiScale();
        base.transform.localScale = Vector3.one * xuiGlobalScaling * @float * activeUiScale;
    ...
    public void Awake()
    {
        ...
        foreach (NGUIFont val in nGUIFonts)
        {
            FontsByName.Add(((UnityEngine.Object)(object)val).name, val);
        }
    }
    ...
    public void Init(int _id)
    {
        ...
        xuiAnchors = (UIAnchor[])(object)new UIAnchor[9];
        UIAnchor[] array2 = anchors;
        foreach (UIAnchor val in array2)
        {
            val.runOnlyOnce = false;
            val.uiCamera = playerUI.camera;
            if (((Component)(object)val).transform.parent.GetComponent<XUi>() == this)
            {
                xuiAnchors[val.side] = val;
            }
        }
    ...
    public Bounds GetXUIWindowWorldBounds(Transform _xuiElement, bool _includeInactive = false)
    {
        ...
        {
            UIBasicSprite val = list[i];
            Transform parent = ((Component)(object)val).transform.parent;
            if (parent != null && parent.name.Equals("MapSpriteEntity(Clone)"))
            {
                continue;
            }

            Vector3[] worldCorners = ((UIRect)val).worldCorners;
    ...
    public UIAnchor GetAnchor(Side _anchorSide)
    {
        //IL_0006: Unknown result type (might be due to invalid IL or missing references)
        return xuiAnchors[_anchorSide];
    }
    ...
    public void UpdateAnchors()
    {
        ...
        foreach (UIAnchor val in array)
        {
            if ((UnityEngine.Object)(object)val != null && !(((UnityEngine.Object)(object)val).name == "AnchorCenterCenter"))
            {
                if ((int)val.side == 5 || (int)val.side == 4 || (int)val.side == 6)
                {
                    val.relativeOffset.x = 0f - num;
                }

                if ((int)val.side == 1 || (int)val.side == 2 || (int)val.side == 0)
                {
                    val.relativeOffset.x = num;
                }

                if ((int)val.side == 3 || (int)val.side == 4 || (int)val.side == 2)
                {
                    val.relativeOffset.y = 0f - num;
                }

                if ((int)val.side == 7 || (int)val.side == 6 || (int)val.side == 0)
                {
                    val.relativeOffset.y = num;
                }

                if ((int)val.side == 7 || (int)val.side == 3 || (int)val.side == 8)
                {
                    val.relativeOffset.x = 0f;
                }
            }
        }
    }
    ...
    public static void HandlePaging(XUi _xui, Func<bool> _onPageUp, Func<bool> _onPageDown, bool useVerticalAxis = false)
    {
        ...
        {
            Vector2 vector = ((TwoAxisInputControl)_xui.playerUI.playerInput.GUIActions.Camera).Vector;
