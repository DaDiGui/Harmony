...
public class NetPackageAddRemoveBuff : NetPackage
{
    ...
    {
        base.write(_writer);
        ((BinaryWriter)_writer).Write(entityId);
        ((BinaryWriter)_writer).Write(buffName);
        ((BinaryWriter)_writer).Write(duration);
        ((BinaryWriter)_writer).Write(adding);
        ((BinaryWriter)_writer).Write(instigatorId);
