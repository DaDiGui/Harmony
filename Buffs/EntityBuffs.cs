...
public class EntityBuffs
{
    ...
    public BuffStatus AddBuff(string _name, Vector3i _instigatorPos, int _instigatorId = -1, bool _netSync = true, bool _fromElectrical = false, float _buffDuration = -1f)
    {
        ...
        if (buff.DamageType != 0 && _instigatorId != parent.entityId && !parent.FriendlyFireCheck(GameManager.Instance.World.GetEntity(_instigatorId) as EntityAlive))
        {
    ...
    {
        NetPackageAddRemoveBuff package = NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(parent.entityId, _name, -1f, _adding: false, -1, Vector3i.zero);
        if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
        {
            SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(package, _onlyClientsAttachedToAnEntity: false, -1, -1, parent.entityId);
        }
    ...
    {
        bool flag = true;
        float value = GetCustomVar(_name);
        switch (_operation)
        {
            case CVarOperation.set:
            case CVarOperation.setvalue:
                if (!CVars.TryGetValue(_name, out value) || value != _value)
                {
                    CVars[_name] = _value;
                }
                else
    ...
    {
        if ((parent.isEntityRemote || _name[0] == '%') && _name[0] != '.' && _name[0] != '_')
        {
            if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
            {
